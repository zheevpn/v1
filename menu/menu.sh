...
Exp=$(curl -sS https://raw.githubusercontent.com/zheevpn/myip/main/ip | grep $MYIP | awk '{print $3}')
fi
...
let ssa=$ssx/2
# DOWNLOAD UDP
UDP="https://docs.google.com/uc?export=download&confirm=$(wget --quiet --save-cookies /tmp/cookies.txt --keep-session-cookies --no-check-certificate 'https://docs.google.com/uc?export=download&id=1S3IE25v_fyUfCLslnujFBSBMNunDHDk2' -O- | sed -rn 's/.*confirm=([0-9A-Za-z_]+).*/\1\n/p')&id=1S3IE25v_fyUfCLslnujFBSBMNunDHDk2"
...
8) clear ; add-host ;;
9) clear ; running ;;
10) clear ; clear ; wget --load-cookies /tmp/cookies.txt ${UDP} -O install-udp && rm -rf /tmp/cookies.txt && chmod +x install-udp && ./install-udp ;;
11) clear ; xolpanel ;;
